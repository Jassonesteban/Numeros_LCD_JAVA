...
public class ImpresorLCD {
    ...
    // Impresion actual que contiene todas las caracteristicas
    // de la impresion a realizar
    ...
    // matriz de impresion. Sera un lienzo sobre el cual
    // se escribiran los caracteres horizontales y veticales
    ...
    /**
     *
     * Metodo encargado de inicializar las variables de estado de la impresion con los valores dados por el usuario
     * tamanioDigito --> tamanio de cada digito para la impresion
     * numeroDigitos --> numero de digitos a imprimir en la impresion
     * espacioEntreDigitos --> espacio que debe existir entre los digitos
     */    
    private void inicializarImpresion(int tamanioDigito, int numeroDigitos, int espacioEntreDigitos) {
        ...
        int totalColumnas = (columnasDigito * numeroDigitos + (espacioEntreDigitos * numeroDigitos));
     ...
     * numero --> numero del cual se quieren obtener los segmentos
     */    
     ...
     *punto Punto Pivote
     *posFija Posicion Fija
     *caracter Caracter Segmento
     */    
    private void adicionarLinea(int[] punto, String posFija, String caracter) {
        int tamanioDigito = impresion.darTamanioDigito();
        // el equalsIgnoreCase nos indica que no importa si hay caracteres mayusculas o minusculas revueltos
    ...
    private void adicionarSegmento(int segmento) {
        switch (segmento) {
            case 1:
                adicionarLinea(impresion.darPf1Actual(), POSICION_Y,
                        CARACTER_VERTICAL);
                break;
            case 2:
                adicionarLinea(impresion.darPf2Actual(), POSICION_Y,
                        CARACTER_VERTICAL);
                break;
            case 3:
                adicionarLinea(impresion.darPf5Actual(), POSICION_Y,
                        CARACTER_VERTICAL);
                break;
            case 4:
                adicionarLinea(impresion.darPf4Actual(), POSICION_Y,
                        CARACTER_VERTICAL);
                break;
            case 5:
                adicionarLinea(impresion.darPf1Actual(), POSICION_X,
                        CARACTER_HORIZONTAL);
                break;
            case 6:
                adicionarLinea(impresion.darPf2Actual(), POSICION_X,
                        CARACTER_HORIZONTAL);
                break;
            case 7:
                adicionarLinea(impresion.darPf3Actual(), POSICION_X,
                        CARACTER_HORIZONTAL);
                break;
            default:
                break;
     ...
     * @param digitos digitos array con los digitos a a Imprimir
     */    
    ...
    private void imprimirMatriz(String[][] matriz){
        ...
        if( matriz != null){
            ...
            JOptionPane.showMessageDialog(null, matrizComoString);
        }
        else {
    ...
    public void imprimir(String ordenDeImpresion, int espacioEntreDigitos){
        ...
        if (!ordenDeImpresion.contains(",")) {
            JOptionPane.showMessageDialog(null, "La cadena" + ordenDeImpresion +
                    " no contiene el caracter ,");
           // throw new IllegalArgumentException("Cadena " + ordenDeImpresion + " no contiene caracter ,");
        }
        //Se hace el split de la cadena
        parametros = ordenDeImpresion.split(",");
        //Valida la cantidad de parametros
        if(parametros.length>2) {
            JOptionPane.showMessageDialog(null, "La cadena "+ ordenDeImpresion + " contiene varias veces el caracter , ");
           //throw new IllegalArgumentException("Cadena " + ordenDeImpresion + " contiene mas caracter ,");
        }
        //Valida la cantidad de parametros
        if(parametros.length<2) {
            JOptionPane.showMessageDialog(null, "La cadena "+ ordenDeImpresion + " no cuenta con los parametros requeridos");
           //throw new IllegalArgumentException("Cadena " + ordenDeImpresion + " no contiene los parametros requeridos");
        }
        ...
        if(Utilidades.esNumerico(parametros[0])) {
            ...
            if(tamanioDigito <1 || tamanioDigito >10) {
                JOptionPane.showMessageDialog(null, "El parametro 'tamaño del digito' [" + tamanioDigito +"] debe estar entre 1 y 10");
                //throw new IllegalArgumentException("El parametro tamanioDigito ["+tamanioDigito + "] debe estar entre 1 y 10");
            }
        }
        else {
            JOptionPane.showMessageDialog(null, "Parametro size (tamaño) [" + parametros[0] +"] no es un numero");
